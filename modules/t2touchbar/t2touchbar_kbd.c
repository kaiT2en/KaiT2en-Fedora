...
static int appletb_kbd_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	kbd->backlight_dev = backlight_device_get_by_name("t2tb_backlight");
	if (!kbd->backlight_dev) {
	...
	kbd->inp_handler.name = "t2touchbar";
...
MODULE_SOFTDEP("pre: t2touchbar_bl");

MODULE_AUTHOR("Ronald Tschalär");
MODULE_AUTHOR("Kerem Karabay <kekrby@gmail.com>");
MODULE_AUTHOR("Aditya Garg <gargaditya08@live.com>");
MODULE_AUTHOR("kait2en");
MODULE_DESCRIPTION("Kait2en T2 Touch Bar Keyboard Mode driver");
MODULE_LICENSE("GPL");
