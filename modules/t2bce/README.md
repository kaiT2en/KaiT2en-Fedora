...
Download the zip file and go into the extracted t2bce folder with terminal. Type
...
sudo reboot
```
After reboot type `modinfo t2bce`. The output should show a version number > `0.04` and `author: André Eikmeyer <andre.eikmeyer@gmail.com>`
 

## DKMS

Install the source tree and register the module with DKMS (t2bce version 0.041 as example here):

```bash
version=0.041
sudo install -d "/usr/src/t2bce-${version}"
git archive --format=tar HEAD | sudo tar -x -C "/usr/src/t2bce-${version}"
sudo dkms add -m t2bce -v "${version}"
sudo dkms install -m t2bce -v "${version}"
```
...
```bash
sudo dkms remove -m t2bce -v 0.041 --all
sudo rm -rf /usr/src/t2bce-0.041
```
